...
class Env2(object):
    ...
    def get_subgraphs_transformer(self, entites, times, query_times, max_action_num=200, first_step=False):
        ...
        subgraph_entitys, subgraph_times, subgraph_rels, subgraph_confs, rela_mats, trgs, lengths = None,None,None,None,None,None,None
        ...
        subgraph_entitys = actions[:,:,1]
        subgraph_times = actions[:,:,2]
        subgraph_rels = actions[:,:,0]

        if self.config['cuda']:
            subgraph_entitys = torch.tensor(subgraph_entitys, dtype=torch.long, device='cuda')
            subgraph_times = torch.tensor(subgraph_times, dtype=torch.long, device='cuda')
            subgraph_rels = torch.tensor(subgraph_rels, dtype=torch.long, device='cuda')
        else:
            subgraph_entitys = torch.tensor(subgraph_entitys, dtype=torch.long)
            subgraph_times = torch.tensor(subgraph_times, dtype=torch.long)
            subgraph_rels = torch.tensor(subgraph_rels, dtype=torch.long)
