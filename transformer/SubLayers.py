...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, link=None):
        ...
        if self.rela_num == 0: # Decode
            ...
            # batch, n_head, length of input, length of input
            if mask is not None:
    ...
    def calcE(self, X, Wq, Wk, Wr, link):
        ...
        return (M*)n*n
        """
        Xq = torch.matmul(X, Wq).reshape(X.size(0), X.size(1), self.n_head, -1).transpose(1,2) # batch, n_head, length of input, dim of embd    其中的-1表示的是将剩下的作为一个维度的值 transpose表示的是将维度进行换位置
        ...
        rep = [1 for _ in range(len(Xk.size())+1)]              # [1, 1, 1, 1, 1]
        rep[-3] = Xk.size(-2)                                   # [1, 1, length of input, 1, 1]
        Xk = Xk.unsqueeze(-3).repeat(rep) + Rk
        Xq = Xq.unsqueeze(-1)

        # print('Xq的size',Xq.size())                              # Xq的size torch.Size([10, 4, 140, 50, 1])
        # print('Xk的size',Xk.size())                              # Xk的size torch.Size([10, 4, 140, 140, 50])
