...
class DataBase():
    def __init__(self, data_path, rel_pad=0, subgraph='') -> None:
        ...
        self.relations = [torch.sparse.FloatTensor(indices[i], values[i], size).coalesce() for i in
                          range(self.pos_rels)]                 # 生成一个稀疏矩阵 其中coalesce表示的是合并相同坐标的内容

        self.filtered_dict = defaultdict(set)                   # 该数据表示为扩充后的三元组
        ...
        for h in self.neighbors:
            self.neighbors[h] = {r: list(ts) for r, ts in self.neighbors[h].items()}    # 生成邻居信息
        ...
        for e in self.e2id.values():
            if e not in self.neighbors.keys():                                          # 不在train训练集中
                self.nebor_relation[e][2 * self.pos_rels] += 1
        self.nebor_relation = torch.log(self.nebor_relation)
        self.nebor_relation /= self.nebor_relation.sum(1).unsqueeze(1)                  # 对数据进行归一化处理
            ...
            for parent in range(length[jump], length[jump + 1]):
                for r in self.neighbors[subgraph[parent]]:
                    ...
                    for t in self.neighbors[subgraph[parent]][r][:MAXN]:
                        try:
                            pos = subgraph.index(t)                                     # 在subgraph中找到t的索引
...
class pickleDataset(torch.utils.data.Dataset):
    ...
    def getsubgraph(self, H, R, T):
        ...
        subgraph += [0 for _ in range(self.padding - len(subgraph))]    # padding填充
        return torch.LongTensor(subgraph), torch.FloatTensor(rela_mat), torch.tensor([H, R, T]), torch.LongTensor(
            [0]), torch.LongTensor(length)
